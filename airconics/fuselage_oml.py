...
#        StarboardCurve = hSt.GetObject()
  ...
  def BuildFuselageOML(self, Max_attempt=5):
    ...
    while i_attempt < Max_attempt:
      ...
      logger.debug("Surface fit attempt {}".format(i_attempt))
      ...
      logger.debug("""Attempting loft surface fit with network density
...
#
          ...
          logger.warning("Intersection Points at Section X={} Not Found"
                      .format(XStation))
          logger.warning("Skipping this plane location")
          continue

        PseudoDiameter = abs(IPoint4.Z() - IPoint2.Z())
        if self.CylindricalMidSection and\
                NoseEndX < XStation < TailStartX:
          logger.debug("Enforcing circularity in the central section...")
          if FirstTime:
            PseudoRadius = PseudoDiameter / 2.
            FirstTime = False
          PseudoRadius = PseudoDiameter / 2.
...
#            Pipe Shell:
    if OMLSurf is None:
      logger.warning("""Failed to fit network surface to the external shape of the
                fuselage""")
      logger.warning("""Attempting alternative fitting method, quality likely to
                be low...""")
