...
def approxThickness(points=None):
    """Approximate airfoil thickness from points on the edge (ordered from
    trailing edge, over the upper surface, then back to the trailing edge)

    Assumes that the input points X values are normalised between zero and one
    (unit chord length), and that the curve has a single point at the location
    of the leading edge. Errors will be larger if this is not the case.
    ...
    points : numpy array (2, N)
        Array of x,y points
    """
    i_LE = points.argmin(axis=0)[0]
    # both XU and XL are the points from the trailing to leading edges
    XU = points[:i_LE + 1, :]
    XL = points[i_LE:, :][::-1]
    interpU = interp1d(XU[:, 0], XU[:, 1], kind='quadratic')
